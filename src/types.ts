...
export interface BitbucketClientConfig {
  /** Bitbucket Personal Access Token for authentication */
  token: string;
  /** Base URL for Bitbucket Server (e.g., 'https://bitbucket.example.com') */
  baseUrl: string;
...
// =============================================================================
// Method Parameter Types - For BitbucketClient Methods
// =============================================================================
...
/**
 * Parameters for getDashboardPullRequests method.
 */
export interface GetDashboardPullRequestsParams {
  /** Filter by role: AUTHOR, REVIEWER, or PARTICIPANT */
  role?: 'AUTHOR' | 'REVIEWER' | 'PARTICIPANT';
  /** Filter by state: OPEN, DECLINED, or MERGED */
  state?: 'OPEN' | 'DECLINED' | 'MERGED';
  /** Filter by participant status (comma-separated): UNAPPROVED, NEEDS_WORK, APPROVED */
  participantStatus?: string;
  /** Sort order: OLDEST, NEWEST, DRAFT_STATUS, PARTICIPANT_STATUS, CLOSED_DATE */
  order?: 'OLDEST' | 'NEWEST' | 'DRAFT_STATUS' | 'PARTICIPANT_STATUS' | 'CLOSED_DATE';
  /** Only return PRs closed within N seconds */
  closedSince?: number;
  /** Specify a different user (defaults to authenticated user) */
  user?: string;
...
// =============================================================================
// Repository Browse Types
// =============================================================================

/**
 * Parameters for browse method.
...
export interface BrowseParams {
  ...
  /** The repository slug */
  repositorySlug: string;
  /** Path to browse (empty or omit for root directory) */
  path?: string;
  /** Branch, tag, or commit ref (e.g., "refs/heads/main") */
  at?: string;
  /** Pagination limit (default: 500) */
  limit?: number;
...
/**
 * Parameters for getRawContent method.
 * Get raw content from repository (plain text for files, git tree listing for directories).
 */
export interface GetRawContentParams {
  /** The Bitbucket project key */
  projectKey: string;
  /** The repository slug */
  repositorySlug: string;
...
/**
 * Entry in browse response (file or directory).
 */
export interface RestBrowseEntry {
  /** Path information */
  path: RestPath;
  /** Entry type: DIRECTORY, FILE, or SUBMODULE */
  type: 'DIRECTORY' | 'FILE' | 'SUBMODULE';
  /** Content ID (present for files) */
  contentId?: string;
  /** Node hash (present for directories) */
  node?: string;
...
/**
 * Response when browsing a directory.
 * Contains children array with file/folder entries.
 */
export interface BrowseDirectoryResponse {
  ...
  children: PaginatedResponse<RestBrowseEntry>;
}

/**
 * Response when browsing a file via /browse endpoint.
 * Contains file content as lines.
 */
...
/**
 * Union type for browse response.
